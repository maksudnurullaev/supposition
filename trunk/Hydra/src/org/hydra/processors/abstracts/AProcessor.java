...
public abstract class AProcessor extends AStatisticsApplyer implements IProcessor {
	...
	public void applyMessage(IMessage inMessage){
		...
		if(isMessageHandlerExist(getMessageClassSimpleName(inMessage))){
			...
			String errorMessage = String.format("Could not find handler for message class(%s) and groupID(%s)" 
					,getMessageClassSimpleName(inMessage)
			...
			inMessage.setObject(Constants.createProxyMessage("message.error", errorMessage));
